...
public class SelectRecords {
    ...
    //query no tinitiated yet. needs to be a query of all the hq platoons and return not the id numbers of the
    // positions in the hq platoon but rather the soldier information related to the ids in hq platoon
    private void queryHQPLATOON2() {
        ...
        try{
            ...
            rs = statement.executeQuery("SELECT m_id, m_name, rank, position" +
                    "FROM soldier, hq_platoon" +
                    "WHERE co_id=m_id OR xo_id=m_id OR fsg_id=m_id OR medic_id=m_id OR rto_id=m_id OR commo_id=m_id" +
                    "OR nbc_id=m_id OR supply_id=m_id ");
            System.out.println("CO  XO  1SG  Medic  RTO  COMMO  NBC  Supply");
            while(rs.next()){
                int m_id = rs.getInt("m_id");
                String m_name = rs.getString("m_name");
                String rank = rs.getString("rank");
                String position = rs.getString("position");
                System.out.println(m_id + "  " + m_name + "  " + rank + "  " + position);
    ...
    private void queryHQ_Platoons() {
        ...
        try {
            ...
            /*rs = statement.executeQuery("SELECT m_id, m_name, component, rank, position " +
                    "FROM soldier, hq_platoon" +
                    " WHERE  co_id = m_id OR xo_id = m_id OR fsg_id = m_id OR medic_id =m_id OR commo_id = m_id");
            */while (rs.next()) {
